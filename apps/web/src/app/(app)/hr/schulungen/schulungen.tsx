...
export function Schulungen({ darfSchreiben, start = "bearbeiten" }: { darfSchreiben: boolean; start?: Ansicht }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-[var(--border)]" role="tablist" aria-label={worte.pfad.seiten["/hr/schulungen"]}>
        {register.map((r) => (
          <button
            key={r.wert}
            type="button"
            ...
            className={cn(
              "-mb-px border-b-2 px-4 py-2 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-[var(--ring)]",
              ansicht === r.wert
