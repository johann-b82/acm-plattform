...
14. **KPI-Registry-Lücken**: Für Ø Auftragswert, Aufträge gesamt, Umsatzwachstum und Kundenanteil gibt es keinen Registry-Key, an diese Kacheln können keine Bubbles/Maßnahmen gehängt werden. Im neuen Stack entfällt die Lücke, weil es kein zweites Register gibt: `/kpi/bewertung` listet `zielwerte` selbst, und der Fremdschlüssel von `kpi_kommentare` und `kpi_massnahmen` geht auf `zielwerte.schluessel` (Migration `0018_kpi_bewertung`). Eine Kennzahl ohne Zielwert lässt sich damit weiterhin nicht kommentieren — das ist jetzt aber dieselbe Liste, die auch die Dashboards prägt, statt einer zweiten, die veraltet.
